...
open IN,$lst or die "$!";
open OUTSH,">>$outdir/$name.mafft.sh" or die "$!";
while(<IN>){
	my ($id1,$id2);
	if (/^(\S+)\s+(\S+)/){
		$id1 = $1;
	...
	my $mafft = "$dir/$subdir1/$subdir2/$id1.$id2.mafft_ginsi.fa";
	...
	$sh_out = "ginsi --anysymbol --thread 3 $sub_pep >$mafft 2>$log;\n";
	print OUTSH $sh_out;
	$loop2++;
}
close IN;
